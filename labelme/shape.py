...
class Shape(object):
    ...
    def scaleTranslate(self, point):
        return QtCore.QPointF(point.x() * self.scale[0], point.y() * self.scale[1])
    ...
    def paint(self, painter):
        ...
        painter.save()
        painter.scale(1 / self.scale[0], 1 / self.scale[1])
        ...
        if self.mask is not None:
            ...
            qimage = qimage.scaled(
                QtCore.QSize(
                    int(qimage.width() * self.scale[0]),
                    int(qimage.height() * self.scale[1]),
                ),
    ...
    def nearestVertex(self, point, epsilon):
        ...
        min_i = None
        point = QtCore.QPointF(point.x() * self.scale[0], point.y() * self.scale[1])
        for i, p in enumerate(self.points):
            p = QtCore.QPointF(p.x() * self.scale[0], p.y() * self.scale[1])
    ...
    def nearestEdge(self, point, epsilon):
        ...
        for i in range(len(self.points)):
            ...
            start = QtCore.QPointF(start.x() * self.scale[0], start.y() * self.scale[1])
            end = QtCore.QPointF(end.x() * self.scale[0], end.y() * self.scale[1])
            line = [start, end]
